  ...
  function onSessionUpdate(update) {
    ...
    switch (kind) {
      ...
      case "agent_message_chunk": {
        ...
        if (!text) return;
        if (!toolsSeen) {
          // Pre-tool narrative — belongs in Working, folded under a note
          // step that reads as process output, not the final answer.
          if (!noteStep) {
            noteStep = { kind: "thinking", title: "Working", text: "", finalized: false, startedAt: Date.now() };
