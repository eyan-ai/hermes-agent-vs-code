...
function createChatParser(handlers) {
  ...
  const isNoise = line => {
    // Trim first: CLI init lines ("  ✅ Enabled toolsets…",
    // "  hermes --resume …") carry leading spaces and must still be
    // classified as noise, not fallback answer content.
    const t = line.trim();
    return /^(Query:|Initializing|─{4,}|🤖|🔗|🔑|✅|🛠️|⚠️|📊|💬|🎉|Resume|Session:|Duration:|Messages:|┌|└|╭|╰|hermes --resume|$)/.test(t);
  };
