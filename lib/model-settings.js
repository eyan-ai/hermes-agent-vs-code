...
function latestRuntimeModelState(sessions, preferredSessionId) {
  const source = Array.isArray(sessions) ? sessions : [];
  const candidates = source
    .filter(session => Array.isArray(session?.modelState?.options) && session.modelState.options.length)
    .sort((left, right) => {
      const availableCount = session => session.modelState.options.filter(option => !option?.unavailable).length;
      const countDifference = availableCount(right) - availableCount(left);
      if (countDifference) return countDifference;
      if (left.id === preferredSessionId) return -1;
      if (right.id === preferredSessionId) return 1;
      return 0;
    });
  const selected = candidates[0]?.modelState;
  if (!selected) return undefined;
  const options = selected.options.map(option => ({ ...option }));
  const requestedCurrent = String(selected.current || "").trim();
  const current = options.some(option => option.id === requestedCurrent)
    ? requestedCurrent
    : options.find(option => !option.unavailable)?.id || options[0]?.id || "";
  return { options, current };
}
...
function normalizeReasoningByModel(raw) {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return {};
  const normalized = {};
  for (const [model, effort] of Object.entries(raw)) {
    const modelIdValue = String(model || "").trim();
    const effortValue = String(effort || "").trim().toLowerCase();
    if (modelIdValue && EFFORT_VALUES.has(effortValue)) normalized[modelIdValue] = effortValue;
  }
  return normalized;
}
...
module.exports = {
  ...
  latestRuntimeModelState,
  mergeRefreshedModels,
  modelId,
  normalizeModelState,
  normalizeReasoningByModel,
